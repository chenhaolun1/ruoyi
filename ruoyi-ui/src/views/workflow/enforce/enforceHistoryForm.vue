<template>
  <div>
    <div>
      <h2>适配实施人：{{form.createName}}</h2>
      <el-form  label-width="80px">
        <el-form-item label="适配实施类型" >
          <el-input v-model="form.type"/>
        </el-form-item>
        <el-form-item label="实施文件">
        ...
        <el-form-item label="实施简述" >
          <el-input v-model="form.reason" />
        </el-form-item>
        <el-form-item label="开始时间">
          <el-input v-model="form.enforceStartTime"/>
        </el-form-item>
        <el-form-item label="结束时间">
          <el-input v-model="form.enforceEndTime"/>
        </el-form-item>
      </el-form>
    </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      getEnforce() {
        getEnforce(this.businessKey).then(response => {
      ...
      handleDownload(realName,filePath) {
        ...
        a.setAttribute("href","http://localhost:80"+process.env.VUE_APP_BASE_API+ "/workflow/enforce/common/download/resource?resource="+suffix+"/"+name)
        a.click()
      }
    }
  }
