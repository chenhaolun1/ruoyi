  ...
  methods: {
    async initChart () {
     ...
     // 获取中国地图的矢量数据
      ...
      const ret = await axios.get('http://localhost:80/static/map/china.json')
      ...
      this.chartInstance.on('click', async arg => {
        ...
        if (!this.mapData[provinceInfo.key]) {
          const ret = await axios.get('http://localhost:80' + provinceInfo.path)
